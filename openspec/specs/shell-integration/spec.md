...
The `shell-integration` capability is the connective tissue between repograph and the shell a developer lives in. It defines two subcommands — `repograph switch <name>` for teleporting into a registered repo via an eval-safe `cd` line, and `repograph completions <shell>` for emitting static completion scripts driven by the live `Cli` struct. Together they cover the Phase 5 polish items: the exact stdout contract for `switch` (one `cd <path>\n` line, single-quoted when the path contains shell metacharacters, POSIX-escaped when the path contains a single quote), the "did you mean" suggestion behavior on unknown names (Levenshtein ≤ 2 with the `floor(len / 2)` ceiling, up to three candidates), the no-`--json` rule, the no-`[agents]`-load rule, and the full clap_complete shell matrix (bash, zsh, fish, powershell, elvish) with shell-specific marker assertions so generated scripts cannot drift from the actual binary. Both commands preserve the project-wide stdout-only contract — diagnostics flow through `tracing` to stderr — and both are documented end-to-end in `README.md` alongside the companion `rg-cd` shell snippets and the per-shell one-time completion install commands.
