...
The `repograph context` capability is the headline output of repograph: it aggregates each in-scope registered repository's metadata (name, canonical path, current branch) and inlines every file matching the user's selected agents' file patterns (from the `init-command` agent registry) into a single agent-facing payload. The capability defines the scope-resolution semantics (default-all / `--workspace` / positional), the bounded pattern-resolution algorithm against the registry, the stable versioned JSON shape (`schema_version: 1`) consumed by downstream AI agents, the TTY Markdown rendering for direct paste into chat clients, the inline-warnings-not-aborts error model, and the exit-code mapping. It composes the `registry-core`, `workspace-support`, and `init-command` capabilities without modifying them.
